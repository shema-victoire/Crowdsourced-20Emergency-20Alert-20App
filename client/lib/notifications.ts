...
export class RwandaNotificationService {
  ...
  public saveSettings(settings: NotificationSettings) {
    ...
    localStorage.setItem('rwanda-emergency-notifications', JSON.stringify(settings));
  }
  ...
  public async sendBrowserNotification(alert: EmergencyAlert, distance?: number) {
    ...
    const urgency = alert.severity === 'critical' ? 'BIKOMEYE' : alert.severity === 'high' ? 'BYIHUSE' : '';
  ...
  public async sendSMSAlert(phoneNumber: string, alert: EmergencyAlert, distance?: number): Promise<boolean> {
  ...
  public shouldNotifyUser(alert: EmergencyAlert, userLocation?: { lat: number; lng: number }): boolean {
  ...
  public async testNotification() {
    const testAlert: EmergencyAlert = {
      ...
      description: 'Iki ni test ya sisitemu yo gutanga amakuru y\'ubwoba. Sisitemu irakora.',
      latitude: -1.9441,
      longitude: 30.0619,

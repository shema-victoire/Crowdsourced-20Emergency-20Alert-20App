...
export class LocationSharingService {
  ...
  public async startLocationSharing(emergencyType?: string, message?: string): Promise<string | null> {
  ...
  public async checkNearbyEmergencies(userLat: number, userLng: number, radiusKm: number = 1): Promise<boolean> {
    try {
      const response = await fetch(`/api/alerts?lat=${userLat}&lng=${userLng}&radius=${radiusKm}`);
